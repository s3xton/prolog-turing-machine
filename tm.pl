...
/** Changes the tape contents and head position given one of the specific actions "left", "right" or "wrt"
...
move(right, L, Ls, R, Rs, _) :- right(L, Ls, R, Rs).


/** These functions ensure that if the head goes beyond the end of the input,
  * a blank will be placed on the list
*/
left([], [], R, [b-k|R]).
left([L|Ls], Ls, Rs, [L|Rs]).

right(L, [Sym|L], [Sym], [b-k]).
right(L, [Sym|L], [Sym|Rs], Rs).
